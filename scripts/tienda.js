function resolverRutaImagen(ruta) {
    ...
    let base = ruta.replace(/^\.\.\//, "").replace(/^\//, "");
    return /\/paginas\//.test(window.location.pathname) ? "../" + base : base;
}

const cervezasPorDefecto = [
    {
...
function obtenerInventario() {
    let inventario = null;
    try {
        inventario = JSON.parse(localStorage.getItem("inventarioTaberna"));
    } catch (e) {
        inventario = null;
    }
    if (!Array.isArray(inventario) || inventario.length === 0) {
        inventario = cervezasPorDefecto;
        localStorage.setItem("inventarioTaberna", JSON.stringify(inventario));
    }
    return inventario;
}

const cervezas = obtenerInventario();
    ...
    col.innerHTML = `
        <div class="card h-100">
            <div class="card-image py-3" style="background-color: #fdfaf6;">
                <img src="${resolverRutaImagen(cerveza.imagen)}" alt="${cerveza.nombre}" style="height: 180px; width: 100%; object-fit: contain;">
            </div>
